...
"$HOME/.local/share/handsoff/venv/bin/python" -m pip install \
  "https://github.com/monzta1/project-handsoff/releases/download/v0.3.11/project_handsoff-0.3.11-py3-none-any.whl"
...
security and bug-fix releases within the same minor line. Pass `--pin v0.3.11` only
...
A project pinned to `0.3.*` accepts the new patch immediately. For an exact pin,
preview and then record the new pin after installing the wheel:

```bash
handsoff upgrade /absolute/path/to/project --to v0.3.11 --dry-run
handsoff upgrade /absolute/path/to/project --to v0.3.11
```

Replace `v0.3.11` with the release being installed.
...
"$HOME/.local/share/handsoff/venv/bin/python" -m pip install --force-reinstall \
  "https://github.com/monzta1/project-handsoff/releases/download/v0.3.10/project_handsoff-0.3.10-py3-none-any.whl"
