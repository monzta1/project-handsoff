...
4. `record-review-findings` (new command, brokered) with no open attempt: opens and closes one as `changes_requested`. With an open attempt, closes it.
...
| `record-review-findings --by REVIEWER --finding "CODE: summary" [--finding ...]` | Supervisor via broker on the Reviewer's behalf | Closes the open attempt as `changes_requested` with structured findings (1..16). Refuses `--by` equal to `implemented_by` (case and whitespace insensitive). Rolls back to Phase 4 or escalates at the cap. Events `review_attempt_closed` (+ `review_cap_escalated` at cap). |
...
- `_invalidate_decisions` never removes `review_round`, closed `review_attempts`, `review_cap_overrides`, or `escalation`. Criterion mutation, evidence recording, symptom resolution, and phase rollback therefore preserve the counter. A criterion mutation while an attempt is open atomically closes it as `abandoned` with finding code `acceptance_changed`; a later reviewer must open a fresh attempt against the new acceptance hash. `record-review` and `record-review-findings` both require the open attempt's `acceptance_hash` to equal the current `acceptance_hash(criteria)` and refuse stale approval even if a malformed caller bypassed the eager abandonment path.
...
Newly recorded `design_review`, `design_approved`, `review`, `deployment_approved`, and regression request records each store required `scope_hash`; their gate checks recompute and compare it alongside the existing design/acceptance/config hashes. Legacy records without `scope_hash` remain schema-valid but are not silently trusted by a scope-aware completion/deployment gate. A one-time `work-items-sync` bootstrap may attach the current derived hash to a pre-#29 `design_review`/`design_approved` pair only when: both records' existing design/config hashes still match; the approval event's recorded `acceptance_sha256` equals the current acceptance file; every effective required item is derived from the chain-anchored criterion tags (tickets contribute display metadata only); and pre/post persistence scope hashes are identical. It commits `scope_binding_bootstrapped` naming both decision timestamps and the hash. Any failed condition clears both decisions and requires fresh independent and human approval. This is the path for this live run's approvals, which necessarily predate the #29 implementation.
