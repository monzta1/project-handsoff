...
The accepted runner writes `.handsoff-regression.json` atomically after each parsed test result. Mission Control displays that telemetry only when both its `request_id` and `command_sha256` match the current or most recently closed request, then shows a live progress bar, done/total and pass/fail/error/skip counts, each worker's current test, and terminal state. This file is generated display state: it is excluded from repository bindings and never grants authority or counts as evidence. A supported `python -m unittest ...` or `python tests/...py` command is enumerated in a clean subprocess, sorted by exact test ID, and dealt deterministically across at most five workers; every worker has a separate log and the configured command retains one merged, shard-ordered durable result. Empty, duplicate, or import-failed inventories fail closed to the original sequential command. Non-unittest commands remain sequential. Timeouts kill the worker process group, and exit 124 takes precedence over other shard failures.
