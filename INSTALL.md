...
"$HOME/.local/share/handsoff/venv/bin/python" -m pip install \
  "https://github.com/monzta1/project-handsoff/releases/download/v0.3.79/project_handsoff-0.3.79-py3-none-any.whl"
...
security and bug-fix releases within the same minor line. Pass `--pin v0.3.79` only
when the project must remain on one exact engine build.

## One command for the whole house

`handsoff update` (README "Updating the house") brings Handsoff, the Miner,
Sentinel and Beakon to their latest releases on this machine and restarts
Fleet; `handsoff update --dry-run` says what it would do. The steps below
remain for a single tool or a pinned version.
...
```bash
handsoff upgrade /absolute/path/to/project --to v0.3.79 --dry-run
handsoff upgrade /absolute/path/to/project --to v0.3.79
```
