...
test("roleWord: actor prefix first, exactly claude- or codex-, else the configured adapter, else null", () => {
  ...
  assert.equal(logic.roleWord("implementer", snap), "claude");
  assert.equal(logic.roleWord("reviewer", snap), "codex");
  assert.equal(logic.roleWord("supervisor", snap), "host");     // no actor: configured adapter
  assert.equal(logic.roleWord("architect", snap), "claude");    // actor beats the configured codex
...
test("the reviewer station is the design reviewer in Phases 1 and 2 and the implementation reviewer otherwise", () => {
  ...
  assert.equal(logic.roleWord("reviewer", snapshot(1, crew)), "codex");
  assert.equal(logic.roleWord("reviewer", snapshot(2, crew)), "codex");
  assert.equal(logic.roleWord("reviewer", snapshot(3, crew)), "claude");
  assert.equal(logic.roleWord("reviewer", snapshot(4, crew)), "claude");
  assert.equal(logic.roleWord("reviewer", snapshot(5, crew)), "claude");
  assert.equal(logic.roleWord("reviewer", snapshot(undefined, crew)), "claude");
  assert.equal(logic.roleWord("reviewer", snapshot("two", crew)), "claude");
...
test("roleTitle joins ROLE, word, model and source, omitting what is missing", () => {
  ...
  assert.equal(logic.roleTitle("implementer", partial), "IMPLEMENTER · codex · recommended");
...
test("the rendered chiclets carry the word and the title for a full snapshot, and the role alone for an empty one", () => {
  ...
  assert.deepEqual(rendered.map((el) => el.text()), ["SUPERVISOR host", "ARCHITECT claude", "IMPLEMENTER claude", "REVIEWER codex"]);
  assert.deepEqual(rendered.map((el) => el.title), ["SUPERVISOR · host · default · explicit", "ARCHITECT · claude", "IMPLEMENTER · claude", "REVIEWER · codex · default · explicit"]);
