...
ISSUE23_TARGETED_COMMANDS = [
    "python3 -m unittest tests.test_handsoff_supervisor.TestAgentReplacement.test_successful_running_replacement_launches_fresh_same_phase_session",
    "python3 -m unittest tests.test_handsoff_supervisor.TestAgentReplacement.test_active_stop_escalates_and_terminalizes_once",
    "python3 -m unittest tests.test_handsoff_supervisor.TestAgentReplacement.test_failed_relaunch_tries_next_fallback_within_cap",
    "python3 -m unittest tests.test_handsoff_supervisor.TestAgentReplacement.test_nonrecoverable_and_unbounded_quality_requests_are_refused",
    "python3 -m unittest tests.test_handsoff_supervisor.TestAgentReplacement.test_handoff_is_derived_bounded_and_secret_free",
    "python3 -m unittest tests.test_handsoff_supervisor.TestAgentReplacement.test_reviewer_independence_and_governance_gates_are_preserved",
]
...
def setUpModule():
    """Keep issue #23 dogfood checks exact; fixtures are normalized below."""
    text = (ROOT / "handsoff.toml").read_text()
    expected = f"commands = {json.dumps(ISSUE23_TARGETED_COMMANDS)}"
    if expected not in text:
        raise SystemExit(
            "This repo's handsoff.toml must list exactly the six issue #23 targeted checks."
        )
