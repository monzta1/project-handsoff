...
Everything the landing [README](../README.md) points at: the workflow, what is enforced, the crew, the features, the release steps, the limitations. Moved here unchanged on 2026-09-21 (#224); the field notes are in [FIELD-NOTES.md](FIELD-NOTES.md).
...
Mission Control presents that normalized telemetry for any application with a sticky top-bar status cluster plus a responsive per-unit or per-shard grid. The cluster replaces the old Pilot-note composer and duplicate bottom dock; it keeps overall completion, completed/total criteria, and active or latest evidenced test state visible while scrolling. The legacy regression file and console remain compatible: regression telemetry is displayed only when both its `request_id` and `command_sha256` match the current or most recently closed request. Both generated files are excluded from repository bindings and never grant authority or count as evidence. A supported `python -m unittest ...` or `python tests/...py` command is enumerated once, sorted by exact test ID, and dealt deterministically across exactly five workers by default. Each worker runs from a separate source snapshot with unique temp, cache, state, service, and port namespaces; snapshots are removed after pass, failure, or timeout. Dashboard cards, CLI output, and normalized state all report `mode`, `worker_count`, per-shard progress, and `fallback_reason`. Explicitly serial-safe unsupported commands use one worker and name the reason; broken or unsafe inventory fails preflight. Serial and five-worker inventories must cover each test exactly once with equivalent outcomes. Timeouts kill the worker process group, and exit 124 takes precedence over other shard failures.

State, journal, ledger, anchor, Fleet registry, and progress replacements use one filesystem durability primitive: content is flushed and fsynced before same-directory replacement, then the containing directory is synced where supported. One `.bak` last-known-good copy is retained and `handsoff doctor --restore-state PATH` validates and restores it. `doctor` and Mission Control report `full` or `best_effort` capability; this is a filesystem guarantee, not a database transaction. Fleet holds one cross-process lock across every registry read-modify-write and returns an actionable timeout without dropping an update.

Reviewer isolation is decided before session reservation and persisted as `reviewer_isolation`: adapter, enforcement, read-only project access, external scratch, bounded subprocesses, sanitized credentials, network policy, decision, reason, and a tamper-evident digest. Codex uses native scratch containment with network denied. An adapter without an enforceable boundary is refused; weaker compatibility requires both `compatibility_mode` and `compatibility_approved` under `[reviewer_isolation]` and is visible in Mission Control.

`[execution] profile = "safe"` is the generated default. This repository alone declares `dogfood` to make its local self-hosting design/deployment waivers explicit. Initialization records the effective posture before any agent starts, governance hashing binds it, and `safe`, `unattended`, `shared`, or `production` profiles reject inherited waivers during configuration load.
...
`snapshot.adaptive_routing` is total and closed. A stored risk class emits an active route even at zero calls, with the policy-selected tier and model but no fabricated session; historical statuses that actually lack a risk class emit `used: false` and `NOT USED`. Routed runs aggregate recorded calls, token usage, derived cost, timing, rounds, and escalation. The `selections` roster identifies the actor, mission purpose, phase, adaptive-or-configured mode, tier, adapter/model, model source, route consistency, reason, and state for every managed session. Claude stream telemetry records the provider-resolved model on the terminal session; a historical `default` session with no provider report emits a null exact model and Mission Control says `Not reported by provider` instead of presenting `default` as a model name. Provider decorations such as `[1m]` are normalized only for route comparison. A mismatch remains visibly reported, uses the reported catalog profile for cost, and counts conservatively as PREMIUM when the provider model is unknown.
