...
**Portable managed-agent output (#58).** Mission Control shows the focused managed session's recent stdout/stderr even when the launching terminal is on another host. The runner line-buffers and redacts output before writing the gitignored `.handsoff-agent-output.json`; protocol messages, exact prompt echoes, authorization headers, and common key/token/password/secret assignments never enter it. Storage is bounded to 160 lines for each of the eight most recent sessions, older entries are released, and terminal sessions retain only that bounded tail. The panel distinguishes `SIGNAL ACTIVE`, `RUNNING · QUIET`, `TRANSPORT DISCONNECTED`, `SESSION FAILED`, and `SESSION COMPLETE`. It is telemetry only: the file is never hashed, logged, or read by a workflow gate, while stdout/stderr still mirror to the launching agent window when attached. SSE watches the side file, so appended lines invalidate the dashboard snapshot immediately; refreshing simply re-renders the cursor-numbered bounded tail without duplicating lines.
