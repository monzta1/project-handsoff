...
These are prose instructions for whichever agent runner or person plays each role. The `[agents]` values are adapter labels; the portable core intentionally does not assume a particular agent CLI. A host integration should launch the agents and require all state mutations to go through this command surface. The Reviewer can independently critique the Phase-2 design and later review implementation; the Supervisor records those distinct decisions with `record-design-review` and `record-review`. The Architect proposes a design and criteria; the human, not the Architect, records final design approval with `design-approve`.
